...
module ChatCodec =
    ...
    let private decodePayload limits (packet: Dreamsleeve.Protocol.Chat.ClientPacket) =
        ...
        | Dreamsleeve.Protocol.Chat.ClientPacket.PayloadOneofCase.OpenSession ->
            ...
            | Ok username, Ok displayName -> Ok { RequestId = packet.RequestId; Command = ChatCommand.OpenSession(username, displayName) }
        ...
        | Dreamsleeve.Protocol.Chat.ClientPacket.PayloadOneofCase.SendChat ->
            ...
            | Ok channel, Ok text -> Ok { RequestId = packet.RequestId; Command = ChatCommand.SendChat(channel, text) }
            | Error error, _ | _, Error error -> Error(domainError error)
    ...
    let decodeClient (codec: ChatCodec) (bytes: byte array) : Result<ChatRequest, ChatCodecError> =
        ...
        if isNull bytes || bytes.Length = 0 then fail None ChatCodecFailure.EmptyPacket
        elif bytes.Length > config.MaxPacketBytes then fail None ChatCodecFailure.PacketTooLarge
        else
            try
                ...
                else decodePayload config.ChatInput packet
    ...
    let private validateResponse config response =
        if responseRequestId response = Some 0UL then Some(ChatCodecFailure.InvalidEnvelope "request_id")
        else
            match response with
    ...
    let encodeServer (codec: ChatCodec) (response: ChatResponse) : Result<byte array, ChatCodecError> =
        ...
        | None ->
            ...
            if packet.CalculateSize() > config.MaxPacketBytes then fail requestId ChatCodecFailure.PacketTooLarge
            else Ok(packet.ToByteArray())
