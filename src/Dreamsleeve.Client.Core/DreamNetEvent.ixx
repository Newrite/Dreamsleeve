...
export struct EventInfo final
{
  ...
  explicit EventInfo(const enet_uint32 data, const ChannelId channelID, const EventType type) : data(data), channelID(channelID), type(type)
  {}
...
export class DreamNetEvent
{
  ...
  explicit DreamNetEvent(EventInfo info, std::optional<DreamNetPeer> peer, std::optional<DreamNetPacket> packet)
      : info(std::move(info)),
        peer(std::move(peer)),
