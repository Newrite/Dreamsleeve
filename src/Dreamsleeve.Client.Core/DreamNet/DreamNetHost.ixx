...
export class DreamNetHost
{
  ...
  static Result TryCreateClient(const ClientConfig config, const std::optional<HostRuntimeConfig>& runtimeConfig = std::nullopt)
  {
    ...
    auto host = enet_host_create(nullptr, config.maxPeers, config.channelLimit, config.inBwLimit, config.outBwLimit);
  ...
  static Result TryCreateServer(const ServerConfig config, const std::optional<HostRuntimeConfig>& runtimeConfig = std::nullopt)
  {
    auto validationResult = ValidateConfig(config);
    if (!validationResult)
    {
    ...
    auto host = enet_host_create(std::addressof(nativeAddress), config.maxPeers, config.channelLimit, config.inBwLimit, config.outBwLimit);
  ...
  EventResult Service(const TimeOutMs timeoutMs)
  {
    if (!IsValid())
    {
      return DreamNetError::MakeUnexpected(DreamNetErrorCode::InvalidHost, "DreamNetHost::Service called on invalid host");
    }
  ...
  EventResult CheckEvents()
  {
    if (!IsValid())
    {
      return DreamNetError::MakeUnexpected(DreamNetErrorCode::InvalidHost, "DreamNetHost::CheckEvents called on invalid host");
    }
  ...
  NetOperationResult BroadcastPushPacket(DreamNetPacket&& packet, const ChannelId channelId)
  {
    if (!IsValid())
    {
      return DreamNetError::MakeUnexpected(DreamNetErrorCode::InvalidHost, "Cannot broadcast packet through invalid DreamNetHost");
    }
  ...
  NetOperationResult ApplyRuntimeConfig(const HostRuntimeConfig& config)
  {
    if (!IsValid())
    {
      return DreamNetError::MakeUnexpected(DreamNetErrorCode::InvalidHost, "Cannot apply runtime config to invalid DreamNetHost");
    }

    if (config.channelLimit)
    {
      auto validationResult = ValidateChannelLimit(*config.channelLimit, "HostRuntimeConfig.channelLimit");
      if (!validationResult)
      {
    ...
    if (config.bandwidthLimit)
    {
      ...
      enet_host_bandwidth_limit(Native(), bandwidthLimit.incoming, bandwidthLimit.outgoing);
    }
  ...
  {
    if (!IsValid()) return DreamNetError::MakeUnexpected(DreamNetErrorCode::InvalidHost, "Cannot initiate connect on invalid host");
    ...
    if (!nativePeer)
    {
      auto message = std::format("Peer is null when try create connect with address: {}", address.ToString());
    ...
    if (!peer)
    {
      auto message = std::format("Can't configure DreamNetPeer after success create connect peer with address: {}", address.ToString());
  ...
  static EventResult EventResultImpl(const ENetEvent& event, const int result, std::string_view function)
  {
    ...
    if (result < 0)
    {
      return DreamNetError::MakeUnexpected(
        ...
        std::format("{} returned a negative result: {}", function, result));
    }
  ...
  static NetOperationResult ValidateConfig(const NetConfig& config)
  {
    ...
    auto channelLimitValidationResult = ValidateChannelLimit(config.channelLimit, "NetConfig.channelLimit");
    if (!channelLimitValidationResult)
    {
  ...
  static NetOperationResult ValidateMaxPeers(const size_t maxPeers)
  {
    if (maxPeers == 0)
    {
      return DreamNetError::MakeUnexpected(DreamNetErrorCode::InvalidConfig, "NetConfig.maxPeers must be greater than 0");
    }
  ...
  static NetOperationResult ValidateChannelLimit(const ChannelLimit channelLimit, const std::string_view fieldName)
  {
    if (channelLimit != 0 && (channelLimit < ENET_PROTOCOL_MINIMUM_CHANNEL_COUNT || channelLimit > ENET_PROTOCOL_MAXIMUM_CHANNEL_COUNT))
    {
      return DreamNetError::MakeUnexpected(
        DreamNetErrorCode::InvalidConfig,
        std::format(
