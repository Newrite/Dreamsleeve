...
export struct PeerTelemetry final
{
  ...
  double PacketLossRatio() const noexcept
  {
    ...
    return scale > 0.0 ? static_cast<double>(packetStats.packetLoss) / scale : 0.0;
  }
...
export class DreamNetPeer
{
  ...
  static Result TryFromNative(ENetPeer* peer, const std::optional<PeerRuntimeConfig>& config = std::nullopt)
  {
    if (!peer)
    {
      return DreamNetError::MakeUnexpected(DreamNetErrorCode::InvalidPeer, "Cannot create DreamNetPeer from null ENetPeer");
    }
  ...
  NetOperationResult ApplyRuntimeConfig(const PeerRuntimeConfig& config) noexcept
  {
    if (!IsValid())
    {
      return DreamNetError::MakeUnexpected(DreamNetErrorCode::InvalidPeer, "Cannot apply runtime config to invalid DreamNetPeer");
    }
    ...
    if (config.throttle)
    {
      ...
      {
        return DreamNetError::MakeUnexpected(DreamNetErrorCode::InvalidThrottleConfig, "ThrottleConfig intervalMs must be non-zero");
      }
    }
    ...
    if (config.pingIntervalMs)
    {
      ...
      {
        return DreamNetError::MakeUnexpected(DreamNetErrorCode::InvalidPingInterval, "Ping interval must be non-zero");
      }
    }
    ...
    {
      const auto& throttle = *config.throttle;
      enet_peer_throttle_configure(Native(), throttle.intervalMs, throttle.acceleration, throttle.deceleration);
    }
  ...
  NetOperationResult PushPacket(DreamNetPacket&& packet, const ChannelId channelId)
  {
    if (!CanSend())
    {
      return DreamNetError::MakeUnexpected(DreamNetErrorCode::InvalidPeerState, "Cannot send packet because peer is not connected");
    }
  ...
  NetOperationResult PushSpan(const DreamNetPacket::DataSpan data, const ChannelId channelId, const PacketFlag flags = PacketFlag::Reliable)
  {
  ...
  ReceiveOperationResult TryReceive()
  {
    ...
    if (!IsValid())
    {
      return DreamNetError::MakeUnexpected(DreamNetErrorCode::InvalidPeer, "Cannot receive packet from invalid DreamNetPeer");
    }

    auto nativePacket = enet_peer_receive(Native(), std::addressof(channelId));
    if (!nativePacket) return std::nullopt;

    auto packet = DreamNetPacket::TryAdoptNative(nativePacket);
    ...
    return ReceiveResult{.packet = std::move(packet.value()), .channelId = channelId};
  }
  ...
  NetOperationResult PushPacketImpl(DreamNetPacket&& packet, const ChannelId channelId)
  {
    if (!packet.IsValid())
    {
      return DreamNetError::MakeUnexpected(DreamNetErrorCode::InvalidPacket, "Cannot send invalid DreamNetPacket");
    }
