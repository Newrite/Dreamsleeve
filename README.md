...
Серверная агентная основа: [Профили, игроки, каналы и реестр](src/Dreamsleeve.Server.Core/README.ru.md).
Команды и результаты проходят через mailbox: MemoryProfileStore в Infrastructure
хранит профили, ChatAgent в Core владеет участниками и ограниченной историей канала.
PlayerAgent владеет персональным входом и состоянием игрока; SessionRegistry
резервирует PlayerId, маршрутизирует команды и упорядочивает общий онлайн. SendChat и подключение этих агентов к ENet ещё предстоят.
