...
Managed-набор содержит 130 тестов: базовые 81 и 49 сценариев
Background/Outbox/Profiles/ChatAgent/PlayerAgent/SessionRegistry. Проверяются отзывчивость mailbox во время I/O, возврат
...
границы mailbox,
занятый/закрытый mailbox ответа, сериализация конкурентных запросов создания профиля,
уникальность ID и сохранение профилей после завершения отдельных потребителей. Тесты синхронизируются
явными gates, с ограниченным ожиданием для обнаружения зависаний.

ChatAgent проверяется через настоящие mailbox: порядок снимка входа и публикаций,
получатели вместе с автором, выход из канала, независимость снимков, ограниченная
...
конкуренцию за PlayerId (включая одинаковый ID при разных именах), отключение во время получения профиля/входа, старые ответы,
...
GetOrCreate проверяется конкурентными отправителями: все получают один сохранённый
профиль без перезаписи имени или лишних ID. Outbox проверяет FIFO, ёмкость вместе
с выполняющейся отправкой, закрытие адресата и отмену владельца.

PlayerAgent проверяет владение положением/Actor Values, независимые снимки,
сброс персонажа и выход. Реестр проверяет изоляцию ошибки профиля и настоящего
исключения дочернего обработчика, завершение перенаправленного чтения при сбое,
независимость каналов отправки, отзывчивость при заполненном сетевом выходе,
предел запросов, уже принятых каналом, и завершение детей перед Completion реестра.
Wire-показания и ENet runtime эти managed-тесты не реализуют и не проверяют.
