...
контракта. Счётчик и таблица ожидающих результатов будут частью клиентского runtime.
...
Сетевой обработчик SessionOpened пока не реализован; Client.Dev использует обычные
операции модели для своего синтетического входа. Модель не содержит специальной
...
межъязыковой ENet echo заново не реализуется. Настоящие серверные обработчики и
подключение codec к Client.Dev остаются следующим шагом.
