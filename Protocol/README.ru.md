...
Это не аутентификация. Хранилище атомарно получает или создаёт профиль по Username
и назначает PlayerId. Реестр резервирует полученный PlayerId до завершения агента
и очистки членства; другой вход с этим ID получает SessionAlreadyOpen и закрывается.
Агентный обработчик входа реализован; подключение к ENet ещё предстоит.
...
| Модель / будущий обработчик сессии | Изменение состояния, конфликты сообщений, членство, уникальность сессии по PlayerId и допустимость команды в текущей сессии |
...
и события онлайна через агентов игроков, профилей и канала; отправка этих результатов по ENet
